...
public sealed class ProductQueryServiceTests
{
    ...
    public async Task GetProductsAsync_RepositoryReturnsProducts_MapsEveryFieldToDto()
    {
        ...
        var sut = new ProductQueryService(repository, new StubEnterpriseContext());
    ...
    [
        ...
        new("product-2", "NL-GEL-030", "Gel Polish - Rose Quartz", "category-3", "Nail Care", "supplier-3", "Luxe Salon Essentials",
            "$9", DomainInventory.ProductStatus.Active, "Long-wear gel polish.", "org-1", "branch-1"),
        new("product-3", "SK-MASK-012", "Renewal Clay Mask 250ml", "category-4", "Skin Care", "supplier-1", "Glow Beauty Supply Co.",
            "$22", DomainInventory.ProductStatus.Active, "Deep-cleansing clay mask.", "org-1", "branch-1"),
    ];
    ...
    [Fact]
    public async Task GetProductsAsync_ProductInDifferentOrganization_IsExcluded()
    {
        var product = new DomainInventory.Product(
            "product-99", "SKU-99", "Other Org Product", "category-1", "Hair Care", "supplier-1", "Glow",
            "$0", DomainInventory.ProductStatus.Active, string.Empty, "org-2", "branch-3");
        var repository = new StubInventoryRepository([product]);
        var sut = new ProductQueryService(repository, new StubEnterpriseContext { CurrentOrganizationId = "org-1", CurrentBranchId = "branch-1" });
    ...
    [Fact]
    public async Task GetProductsAsync_ProductInDifferentBranchSameOrganization_IsExcluded()
    {
        var product = new DomainInventory.Product(
            "product-98", "SKU-98", "Other Branch Product", "category-1", "Hair Care", "supplier-1", "Glow",
            "$0", DomainInventory.ProductStatus.Active, string.Empty, "org-1", "branch-2");
        var repository = new StubInventoryRepository([product]);
        var sut = new ProductQueryService(repository, new StubEnterpriseContext { CurrentOrganizationId = "org-1", CurrentBranchId = "branch-1" });
