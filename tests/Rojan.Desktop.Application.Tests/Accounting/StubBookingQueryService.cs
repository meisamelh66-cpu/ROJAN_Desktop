...
/// <summary>Minimal <see cref="IBookingQueryService"/> test double - only <see cref="GetBookingsAsync"/> is exercised by <see cref="InvoiceQueryServiceTests"/>.</summary>
internal sealed class StubBookingQueryService : IBookingQueryService
{
    ...
    public Task<IReadOnlyList<BookingDto>> SearchBookingsAsync(BookingSearchFilter filter, CancellationToken cancellationToken = default) =>
        throw new NotSupportedException("Not used by InvoiceQueryServiceTests.");
}
