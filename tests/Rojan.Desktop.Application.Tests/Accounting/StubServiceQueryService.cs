...
/// <summary>Minimal <see cref="IServiceQueryService"/> test double - only <see cref="GetServicesAsync"/> is exercised by <see cref="InvoiceQueryServiceTests"/>.</summary>
internal sealed class StubServiceQueryService : IServiceQueryService
{
    ...
    public Task<IReadOnlyList<ServiceCategoryDto>> GetCategoriesAsync(CancellationToken cancellationToken = default) =>
        Task.FromResult<IReadOnlyList<ServiceCategoryDto>>([]);
}
