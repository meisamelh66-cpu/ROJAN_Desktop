...
/// <summary>Minimal <see cref="ICustomerQueryService"/> test double - only <see cref="GetCustomersAsync"/> is exercised by <see cref="BookingWorkflowServiceTests"/>, so <see cref="SearchCustomersAsync(string, CancellationToken)"/> just delegates to it.</summary>
